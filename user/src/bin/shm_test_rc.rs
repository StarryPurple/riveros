...
use user_lib::{shm_alloc_page, shm_free_page, shm_ref_page, shm_unref_page, shm_gc_collect};
...
pub fn main() -> i32 {
    ...
    // 5. Explicit GC collect — should find nothing (was already collected inside step 4)
    let freed = shm_gc_collect();
    println!("[5] gc_collect: {} pages (0 = already freed in step 4)", freed);

    // 6. Re-allocate — should get our page back (or another if freelist reordered)
    ...
    // Cleanup
    if p1 as i32 >= 0 && shm_alloc_page() < 0 {
        // nothing to clean - the test already freed its pages
    }
